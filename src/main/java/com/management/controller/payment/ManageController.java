...
public class ManageController {
    ...
    public String waterPost(Integer money, HttpSession session){
        ...
        payService.addManage(pay,money);
        return "redirect:pay";
    }

    //管理员进入缴费页面
    @GetMapping("/payment_manage/{id}")
    public String waterManageGet(@PathVariable("id") Integer id, Model model){
        model.addAttribute("id",id);
        return "account/payment/manage";
    }

    //管理员给业主缴费
    @PostMapping("payment_manage/{id}")
